...
public class PanelLoading extends javax.swing.JLayeredPane {
    ...
    public void doneLogin(User data) throws IOException {
        isMessage = false;
        this.data = data;
        ImageIcon imageIcon;
        byte[] blob = data.getImage();
        BufferedImage userImage = ImageIO.read(new ByteArrayInputStream(blob));
        imageIcon = new ImageIcon(userImage);
         
         imageIcon = new ImageIcon(userImage);
        pic.setIcon(imageIcon);
        cmdContinue.setText("Continue with " + data.getLastName());
        animator.start();
    }
